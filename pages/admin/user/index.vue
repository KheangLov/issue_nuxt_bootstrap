...
export default {
  ...
  async asyncData(context) {
    ...
    const sz = 10;
    const { items, total, page, size } = await reqInstance.get(`${process.env.API_URL ? process.env.API_URL : 'https://fastapi-kheanglov.cloud.okteto.net'}/users?page=${pg}&size=${sz}`).then(({ data }) => data);
